...
function FieldLabel({ children, theme, htmlFor, hint }) {
  return (
    <label
      htmlFor={htmlFor}
      className={`mb-1 flex items-center justify-between text-xs font-medium sm:text-sm ${
        theme === "dark" ? "text-gray-300" : "text-gray-700"
      }`}
    >
      <span>{children}</span>
      {hint && (
        <span className={`text-[10px] font-normal sm:text-xs ${theme === "dark" ? "text-gray-500" : "text-gray-400"}`}>
          {hint}
        </span>
      )}
    </label>
      ...
      <div className="mb-5 flex flex-col gap-3 sm:mb-6 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {isStudent ? <GraduationCap size={12} /> : <UserRound size={12} />}
              {roleLabel}
            </span>
            {memberSince && (
              <span
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {!editing ? (
            ...
            <button
              ...
              className={dangerButton(theme, "px-3 py-2 text-xs sm:text-sm")}
            >
              Cancel Editing
            </button>
          )}
          ...
          <div className="mb-5 flex min-w-0 flex-col gap-4 overflow-visible border-b pb-5 sm:mb-6 sm:flex-row sm:items-center sm:justify-between sm:gap-5 sm:pb-6 border-inherit">
            <div className="flex min-w-0 items-center gap-3 sm:gap-4">
              ...
              <div className="min-w-0 flex-1 overflow-hidden">
                ...
                <p className={`mt-0.5 truncate text-xs sm:text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                  {profile.email}
                </p>
                {displaySchoolId && (
                  <p className={`mt-0.5 truncate text-[11px] font-medium ${theme === "dark" ? "text-emerald-400/80" : "text-teal-700"}`}>
                    ID · {displaySchoolId}
                  </p>
                )}
              </div>
            ...
            <div className="shrink-0">
              ...
              <label
                ...
                className={`cursor-pointer ${secondaryButton(theme, "inline-block px-3 py-1.5 text-xs sm:text-sm")} ${avatarUploading ? "pointer-events-none opacity-60" : ""}`}
              >
                {avatarUploading ? "Uploading..." : "Change photo"}
              </label>
              {hasCustomProfilePhoto(editProfile.avatar_url) && (
                <p className={`mt-1.5 text-[11px] ${theme === "dark" ? "text-gray-500" : "text-gray-500"}`}>
